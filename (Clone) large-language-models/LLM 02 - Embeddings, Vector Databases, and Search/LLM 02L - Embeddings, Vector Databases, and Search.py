...
talks_collection.add(
    documents=dais_pdf["Title"][:len(dais_pdf)].tolist(),
    metadatas=[{"Abstract": Abstract} for Abstract in dais_pdf["Abstract"][:len(dais_pdf)].tolist()],
    ids=[f"id{x}" for x in range(len(dais_pdf))],
)
...
question = "What's the latest news on data architectures"
